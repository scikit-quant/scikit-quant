...
def opti_by_gp(dim, bounds,objective, maxevals, histfilename='pointlog.txt'):
    ...
    n_GP_samples = min(20*(dim+1),maxevals-ninit) #max number of GP-based samples we allow -- tunable
    print("n_GP_samples:", n_GP_samples)
    #n_GP_samples = min(1000, maxevals-ninit)
    ...
    print("BUDGET:", maxevals - Xsamples.shape[0])
    ...
    if budget > 0:
        ...
        for ii in range(NUM_DATAPOINTS):
            ...
            close = np.round(startp[ii,:], 1).flatten()
            bounds = np.zeros((len(close), 2))
            bounds[:,0] = np.subtract(close, 0.05)
            bounds[:,1] = np.add(     close, 0.05)
            print("bounds:", bounds)
