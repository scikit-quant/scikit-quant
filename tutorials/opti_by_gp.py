...
def opti_by_gp(dim, bounds,objective, maxevals, histfilename='pointlog.txt'):
    ...
    if budget > 0:
        ...
        for ii in range(NUM_DATAPOINTS):
            ...
            result = optimizer.minimize(objective, np.ravel(startp[ii,:]), bounds=bounds)

            #print(res_x(result), len(res_x(result)))
            '''
            f= open(histfilename,"a+") #save new input-output pair to file
            for jj in range(res_x(result).shape[0]):
                for kk in range(dim+1):
                    f.write("%f  " % (res_x(result)[jj,kk]))
                f.write("\n" )
            f.close()
            '''
            budget = budget - res_nevals(result)
            ...
            if res_energy(result)<opt_y:
                print('found improvement!')
                opt_y = res_energy(result)
                opt_x = res_x(result)
        ...
        print(result )
        try:
            result.nfev = min(maxevals, maxevals-budget)
        except AttributeError:
            r = list(result)
